        ...
        return (
            <>
                {firstLevelMenu.map(c =>{
                    console.log(c)
                    console.log(menu[0].pages[0]._id)
                    return(
                        <div key={c.route}>
                            <>
                ...
                {menu.map(q => {
                    ...
                    return (
                        <div key={q._id.secondCategory}>
                            ...
                            <motion.div variants={variants} layout initial={q.isOpened ? 'visible' : 'hidden'}
    ...
    const buildThirdLevel = (pages: PageItem[], route: string) => {
        return pages.map(p => (
            <motion.div key={p._id} variants={variantsChildren}>
                <Link href={`/${route}/${p._id}`} className={cn(styles.thirdLevel, {
